...
var (
	//testConfigFile = filepath.Join(os.Getenv("GOPATH"), "src/github.com/zhcppy/fabricli", "fixtures/config/config.yaml")
	testConfigFile = filepath.Join(os.Getenv("GOPATH"), "src/github.com/securekey/fabric-examples/fabric-cli", "test/fixtures/config/config_test_local.yaml")
)
...
func Test_ConfigFile(t *testing.T) {
	info, err := os.Stat(testConfigFile)
	if err != nil {
		t.Fatal(err)
	}
	if info.IsDir() {
		t.Fatal(testConfigFile, "is not config file")
	}
}
...
func TestCreate_a_channel(t *testing.T) {
	...
	run("faclie channel create --cid mychannel --config " + testConfigFile)
}
...
func TestQuery_info(t *testing.T) {
	header("Query info")
	run("faclie query info --cid orgchannel --config " + testConfigFile)
}
