...
func (f *Formatter) printfColor(c *color.Color, format string, args ...interface{}) string {
...
func (f *Formatter) printColor(c *color.Color, s string) string {
	if f.DisabledColor || c == nil {
		return fmt.Sprint(s)
	}
...
func (f *Formatter) pretty(v interface{}, depth int) string {
	...
	case float64:
		return f.printColor(f.NumberColor, strconv.FormatFloat(val, 'f', -1, 64))
	case json.Number:
		return f.printColor(f.NumberColor, string(val))
	case bool:
		return f.printColor(f.BoolColor, strconv.FormatBool(val))
	case nil:
		return f.printColor(f.NullColor, "null")
...
func (f *Formatter) processString(s string) string {
	...
	return f.printColor(f.StringColor, s)
}
...
func (f *Formatter) processMap(m map[string]interface{}, depth int) string {
	...
	for _, key := range keys {
		val := m[key]
		k := f.printfColor(f.KeyColor, `"%s"`, key)
