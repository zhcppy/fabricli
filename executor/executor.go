...
import (
	...
	"github.com/zhcppy/fabricli/executor/worker"
...
func (e *Executor) Submit(task worker.Task) error {
	...
	logger.L().Debugf("Submit[%s] - submitting task...\n", e.name)
	e.wg.Add(1)
	e.tasks <- task
	logger.L().Debugf("...Submit[%s] - submitted task\n", e.name)
...
func (e *Executor) SubmitDelayed(task worker.Task, delay time.Duration) error {
	...
	logger.L().Debugf("Submit[%s] - submitting task to execute in %s ...\n", e.name, delay)
...
func (e *Executor) Stop(wait bool) bool {
	...
	logger.L().Debugf("[%s] Stopping executor ...\n", e.name)

	// Wait for the dispatcher to purge its queue
	e.wg.Wait()

	logger.L().Debugf("[%s] Stopping the dispatcher ...\n", e.name)

	// Stop the dispatcher
	e.terminating <- true

	logger.L().Debugf("[%s] Stopping the worker pool ...\n", e.name)

	// Stop the worker pool
	e.pool.Stop(wait)

	logger.L().Debugf("[%s] ... executor stopped.\n", e.name)
...
func (e *Executor) dispatch() {
	for {
		...
		case <-e.terminating:
			logger.L().Debugf("[%s] ... executor dispatcher ended\n", e.name)
			return
		}
	}
}
