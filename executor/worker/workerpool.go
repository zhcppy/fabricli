...
func (p *Pool) Stop(wait bool) {
	logger.L().Debugf("[%s] Stopping worker pool ...\n", p.name)

	if wait {
		// Wait for all the tasks to complete
		logger.L().Debugf("[%s] ... waiting for tasks to complete ...\n", p.name)
		p.taskWg.Wait()
	} else {
		logger.L().Debugf("[%s] ... forcing all tasks to stop ...\n", p.name)
	}

	// Shut down the workers
	logger.L().Debugf("[%s] ... stopping workers ...\n", p.name)
...
func (p *Pool) Submit(task Task) {
	logger.L().Debugf("worker pool.Submit[%s] - waiting for available worker\n", p.name)
	...
	logger.L().Debugf("worker pool.Submit[%s] - got worker [%s]. Submitting task...\n", p.name, w.Name())

	// Submit the task to the worker
	w.Submit(task)

	logger.L().Debugf("worker pool.Submit[%s] - submitted task to worker[%s]\n", p.name, w.Name())
}
...
func (p *Pool) StateChange(w *Worker, state State) {
	...
	case STOPPED:
		logger.L().Debugf("...Worker[%s] stopped\n", w.Name())
		p.wg.Done()
		break

	default:
		logger.L().Warnf("Unsupported worker state: %d\n", state)
		break
	}
}
